...
class UpdateCompleteQuizCountView(APIView):
    ...
    def post(self, request):
        ...
        if QuizAttemptHistory.objects.filter(course_id=course_id, enrolled_user_id=user_id, complete=True).exists():
            ...
            return Response({'message': 'No completed quiz found for the user and course'}, status=status.HTTP_404_NOT_FOUND)
...
class CountOfAssignedCoursesView(APIView):
    ...
    def get(self, request):
        # Assuming user is available in the request
        user_id = request.query_params.get('user_id')
        if not user_id:
            return Response({'error': 'user_id is required'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({'active_enrollments_count': active_enrollments_count})
...
class CountClientCompletedCourseView(APIView):
    ...
    def get(self, request):
        ...
        return Response({'completed_courses_count': completed_courses_count, 'in_progress_courses_count': in_progress_courses_count,'not_started_courses_count': not_started_courses_count})

def employee_dashboard(request):
    # Assuming the API is hosted at a specific URL
    api_url ='http://127.0.0.1:8000/lms/count-client-completed-courses'
    # Assuming user_id is available in the request
    user_id = request.GET.get('user_id')
    if not user_id:
        return render(request, 'error.html', {'error': 'user_id is required'})
    ...
    if response.status_code == 200:
        ...
        return render(request, 'error.html', {'error': 'Failed to fetch data from API'})
# =================================================================
# employer dashboard
# =================================================================
